...
function HousingCard({ housing }) {
  const housingId = housing?.housingId ?? housing?.id;
  ...
  const busy = favorites ? favorites.isBusy(housingId) : false;

  const title = housing.title || "Student Housing";
  const price = Number(housing.price) || 0;
  const rating = Number(housing.rating) || 0;
  const distance = Number(housing.distanceKm);
  const isVerified = housing.isVerified === true;
  ...
  const handleFavorite = async (e) => {
    ...
    if (!isAuthenticated) {
      navigate("/login", { state: { from: "/housing" } });
      return;
    }

    if (!favorites || busy) return;
    await favorites.toggleFavorite(housingId);
  };
    ...
    <div className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="relative h-52 overflow-hidden bg-gradient-to-br from-blue-100 to-slate-200">
        {image ? (
          ...
          <div className="flex h-full items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-sm font-medium text-slate-500">Student Housing</p>
            </div>
          </div>
        )}
        ...
        <button
          ...
          disabled={busy}
          className={`absolute right-4 top-4 flex h-10 w-10 items-center justify-center rounded-full bg-white/95 shadow transition disabled:cursor-wait ${
      ...
      {/* Content */}
      <div className="p-5">
        <h3 className="line-clamp-1 text-lg font-bold text-slate-900">{title}</h3>
        ...
        <div className="mt-5 flex items-end justify-between gap-3">
          <div>
            ...
            <span className="ml-1 text-sm text-slate-500">EGP / month</span>
          </div>
